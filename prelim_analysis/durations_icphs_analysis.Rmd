...
p <- df_check %>%
  mutate(
    ...
    language = fct_relevel(language, "English", "Vietnamese", "Turkish", "Swahili", "Georgian", "Amharic", "Mandarin", "Korean")
    ) %>%
  ...
  scale_color_manual(values = c("Korean" = "#76B7B2", "Mandarin" = "#59A14F", "Amharic" = "#4E79A7",
...
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/check.pdf", width = 3, height = 2,
         bg = "transparent")
...
p1 <- df_check %>%
  ...
  ggrepel::geom_text_repel(aes(label = weight), max.overlaps = 100, box.padding = unit(0.75, "lines"), point.padding = unit(0, "lines"), color = "black", size = 3, segment.size = 0.25, min.segment.length = 0) +
  labs(x = "Proportion of OK alignments", y="") +
  ...
  scale_color_manual(values = c("Korean" = "#76B7B2", "Mandarin" = "#59A14F", "Amharic" = "#4E79A7", "Georgian" = "#E15759", "Swahili" = "#EDC948", "Turkish" = "#B07AA1", "Vietnamese" = "#FF9DA7")) +
  theme(
...
p2 <- ggplot() +
  theme_minimal(base_size = 8) +
  geom_line(data = df_logit, aes(x = X, y = Y), inherit.aes = FALSE, color = "gray50", size = 1) +
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5),
    panel.grid.minor = element_blank(),
    plot.margin = margin(t = 6, r=0, b = 0, l = 17),
    plot.background = element_rect(fill = "transparent", color = NA),
    panel.background = element_rect(fill = "transparent", color = NA),
    legend.position = "none",
    legend.background = element_rect(fill = "transparent", color = NA),
...
ggarrange(p1, p2, nrow=2, heights=c(0.7,0.3))
ggsave(filename = "./graphs_icphs/check_w.pdf", width = 3, height = 3,
       bg = "transparent")
...
p <- d_c_freq %>%
  ...
  coord_flip() +
  labs(y = "Proportion") +
  scale_fill_manual(values = c("Korean" = "#76B7B2", "Mandarin" = "#59A14F", "Amharic" = "#4E79A7",
                                "Georgian" = "#E15759", "Swahili" = "#EDC948", "Turkish" = "#B07AA1",
...
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/seg_freq.pdf", width = 8.4, height = 3.5,
         bg = "transparent")
...
p <- d_uttr_plot %>%
  ...
  geom_point(alpha = 0.1, pch=16) +
  # geom_ribbon(data = filter(newdat_cv_full_plot,
  #                           segment_type_o == "vowel"),
  #             aes(ymin = ll, ymax = ul), y=NA, col = NA, fill = "grey",
  #             alpha = 0.5) +
  # geom_ribbon(data = filter(newdat_cv_full_plot,
  #                           segment_type_o == "consonant"),
  #             aes(ymin = ll, ymax = ul), y=NA, col = NA, fill = "grey",
  #             alpha = 0.5) +
  geom_line(data = filter(newdat_cv_full_plot,
                          segment_type_o == "vowel"),
            aes(y=dur),
            col = "darkorange1", lwd = 1) +
  geom_line(data = filter(newdat_cv_full_plot,
...
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/all_CV.pdf", width = 4.85, height = 6.5,
         bg = "transparent")
#-----
...
p <- eff_sizes_df %>%
  mutate(
    language = recode(language, "Taiwan Mandarin" = "Mandarin", "Seoul Korean" = "Korean", "American English" = "English"),
    model = recode(model, "cv_mod_baseline" = "baseline", "cv_mod_weighted" = "weighted", "cv_mod_full" = "full"),
    model = fct_relevel(model, "baseline", "weighted", "full")) %>%
...
p
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/eff_size_parallel.pdf", width = 2.5, height = 2.5, bg = "transparent", device = cairo_pdf)
...
p <- d_uttr_plot %>%
  ...
  geom_textline(
    ...
    aes(y = dur), color = "#F28E2B", label = "V", size = 5, linewidth = 1, hjust = 0.8) +
  geom_textline(
    data = newdat_all %>%
    ...
    aes(y = dur), color = "#4E79A7", label = "C", size = 5, linewidth = 1, hjust = 0.8) +
  facet_grid(. ~ model) +
  geom_text(
    ...
    aes(label = paste0("Δβ = ", round(eff_size,2)), x = 0.05, y = 0.13), color = "black", size = 3) +
  labs(x = "Average segment duration (s) [inverse speech rate]", y = "Average C vs. V duration (s)") +
  scale_x_log10(breaks = c(0.05, 0.07, 0.1, 0.14)) +
  scale_y_log10(breaks = c(0.05, 0.07, 0.1, 0.14), limits = c(0.03, 0.19)) +
  scale_color_manual(values = c("cons_dur_uttr" = "#365373", "vowel_dur_uttr" = "#c4680c"), guide = "none") +
  theme(
    axis.title = element_text(face="bold"),
    axis.text = element_text(colour="black"),
    plot.title = element_text(hjust = 0.5),
...
p
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/amharic.pdf", width = 6, height = 2.5,
