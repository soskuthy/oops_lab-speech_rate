...
d_drc %>% write_csv("../doreco_data/DoReCo.csv")

d_drc_unique_phones <- d_drc %>%
  group_by(language) %>%
  reframe(
    phone_unique <- unique(phone),
    ipa_unique <- unique(phone_ipa)
  )
...
d %>%
  group_by(language, phone, c_type) %>%
  summarize(
    N = n()
  ) %>%
  write_csv("../data/phone_mapping.csv")

unique(d$c_type)

d %>%
  filter(language == "Vietnamese", segment_type == "vowel") %>%
  pull(phone) %>%
  unique() %>%
  str_subset("^.ə")
  #str_subset("^[iɨu][^ə]")
  
unique(df.raw.drc$category)
```
...
threshold_uttr_min <- 0.04 # 40 ms
threshold_uttr_max <- 0.2 # 200 ms vs. [v] 250 ms

threshold_individual_min <- 0.03  # changed from 0.02
...
d_seg <- d_seg %>% 
  ...
  # change the following to by-language basis
  filter(
...
# remove utterances with too little data left
d_seg <- d_seg %>%
  group_by(language, speaker, uttr_id) %>%
  mutate(
    uttr_dur = sum(phone_dur),
...
p
ggsave2(filename = "./graphs/pop_lang_talk.svg", plot = p, width = 9.42, height = 9, bg = "transparent")

library(latex2exp)
p <- newdat_cv %>%
  ggplot(aes(x = segment_dur_uttr, y = dur, color = segment_type_o)) +
    geom_line(aes(color = segment_type_o), size = 2.5) +
    scale_color_manual(values=c("#56B4E9", "#e69f00"), guide = "none") +
    xlab(TeX("faster $\\leftarrow$ Avg. seg. dur. (1/rate) $\\rightarrow$ slower")) +
    ylab(TeX("shorter $\\leftarrow$ Avg. C/V dur. $\\rightarrow$ longer")) +
    theme(
      panel.grid.major = element_blank(),
      panel.grid.minor = element_blank(),
      plot.background = element_rect(fill = "transparent", color = NA),
...
ggsave2(filename = "./graphs/explain_talk.svg", plot = p, width = 9.42, height = 9, bg = "transparent")
```
...
theme_set(
  theme_bw() +
    theme(
      text = element_text(size = 28, color = "white"),
      plot.background = element_rect(fill = "transparent", color = NA),
...
newdat_cv_full_plot <- newdat_cv_full %>%
  ...
  mutate(
    language = recode(language, "Taiwan Mandarin" = "Mandarin", "Seoul Korean" = "Korean", "American English" = "English"),
    language = factor(language, levels = c(
      "Kapampangan", "Korean", "Mandarin", "Amharic", "Georgian", "Swahili", "Turkish", "Vietnamese", "English"
      )
    )
  )
...
(p <- d_uttr_plot %>%
  ...
  ggplot(data = ., aes(x = segment_dur_uttr, y = value, col = measure)) +
    #coord_equal(ylim = c(0.03, 0.19)) +
    facet_wrap(. ~ language, ncol = 3) +
    #geom_ribbon(data = newdat_cv_full_plot, aes(x = segment_dur_uttr, y = dur, ymin = ll, ymax = ul, group = segment_type_o), color = NA, fill = "grey", alpha = 0.25) +
    geom_point(alpha = 0.2, pch = 16) +
    geom_textline(data = filter(newdat_cv_full_plot, segment_type_o == "vowel"),
                  aes(y = dur), label = "V", size = 7.5, hjust = 0.8, col = "#e69f00", linewidth = 1) +
  geom_textline(data = filter(newdat_cv_full_plot, segment_type_o == "consonant"),
                  aes(y = dur), label = "C", size = 7.5, hjust = 0.8, col = "#56B4E9", linewidth = 1) +
  #coord_cartesian(ylim = c(0.03, 0.19)) +
  scale_x_log10(breaks = c(0.05, 0.07, 0.1, 0.14),
                labels = c(".05", ".07", ".10", ".14")) +
  scale_y_log10(breaks = c(0.05, 0.07, 0.1, 0.14),
                labels = c(".05", ".07", ".10", ".14")) +
  scale_colour_manual(values = c("#177ab2", "#b37c00"), guide = "none") +
  #scale_colour_manual(values = c("purple", "orange"), guide = "none") +
  geom_text(
    data = eff_sizes_df %>%
      filter(model == "cv_mod_full") %>%
    ...
    language = fct_reorder(language, eff_size),
    model = recode(model, "cv_mod_baseline" = "baseline", "cv_mod_weighted" = "alignment", "cv_mod_full" = "alignment + segment")),
    aes(label = paste0("Δβ = ", round(eff_size, 2)), x = 0.05, y = 0.13), color = "white", size = 7.5, inherit.aes = F) +
    #aes(label = paste0("Δβ = ", round(eff_size, 2)), x = 0.05, y = 0.13), color = "black", size = 2, inherit.aes = F) +
  xlab("Average segment dur. in s (inverse speech rate)") +
  labs(y = "Average duration in s") +
  theme(
      panel.grid.major = element_blank(),
      panel.grid.minor = element_blank(),
      plot.background = element_rect(fill = "transparent", color = NA),
...
ggsave2(filename = "./graphs/indiv_lang_talk.svg", plot = p, width = 9.42, height = 9, bg = "transparent")
