...
set.seed(2022)
ggsave(plot = p, filename = "./graphs_icphs/check_w.pdf", width = 3, height = 3,
       bg = "transparent")
...
newdat_cv_full_plot <- newdat_cv_full %>%
  filter(language != "Kapampangan", model == "cv_mod_full") %>%
  mutate(
    language = recode(language, "Taiwan Mandarin" = "Mandarin", "Seoul Korean" = "Korean", "American English" = "English"),
    language = factor(language, levels = c(
      "Korean", "Mandarin", "Amharic", "Georgian", "Swahili", "Turkish", "Vietnamese", "English")))
...
p <- d_uttr_plot %>%
  ...
  geom_textline(data = filter(newdat_cv_full_plot,
            ...
            col = "darkorange4", linewidth = 0.5) +
  geom_textline(data = filter(newdat_cv_full_plot,
                          segment_type_o == "consonant"),
            aes(y = dur), label = "C", size = 3, hjust = 0.8,
            col = "purple4", linewidth = 0.5) +
...
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/all_CV.pdf", width = 3.25, height = 3.25, bg = "transparent")
#-----
...
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/eff_size_parallel.pdf", width = 2.5, height = 2.75, bg = "transparent", device = cairo_pdf)
...
p <- d_uttr_plot %>%
  ...
  geom_textline(
    ...
    aes(y = dur), color = "darkorange4", label = "V", size = 3, linewidth = 0.5, hjust = 0.8) +
  geom_textline(
    data = newdat_all %>%
    ...
    aes(y = dur), color = "purple4", label = "C", size = 3, linewidth = 0.5, hjust = 0.8) +
  facet_grid(. ~ model) +
  geom_text(
...
p %>%
  ggsave(plot = ., filename = "./graphs_icphs/amharic.pdf", width = 6, height = 2,
