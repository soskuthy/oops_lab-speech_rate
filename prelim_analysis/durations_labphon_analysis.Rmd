...
plot_b <- ggplot(filter(d_uttr_plot, measure_type=="dur"), aes(x=segment_dur_uttr, y=value, col=measure)) +
  facet_grid(. ~ language) +
  geom_point(alpha=0.3) +
  geom_ribbon(data=filter(newdat, 
                          measure_type=="dur",
...
ggplot(newdat, aes(x=log_segment_dur_uttr, y=value, col=segment_type_f)) +
  ...
  theme(panel.grid = element_blank(),
        ...
        strip.placement = "outside",
        strip.text=element_text(size=16, face="bold"))
ggsave("graphs/labphon_ctype_mod_preds.png", width=7.5, height=4, dpi=300)
```